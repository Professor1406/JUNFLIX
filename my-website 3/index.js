// Consts
const apikey = "648c004c97b5a1425c702528ab88ddac";
...
function buildBannerSection(movie) {
    ...
    div.innerHTML = `
            ...
            <div class="action-buttons-cont">
                <button class="action-button" onclick="playMovie('${movie.title}', '${movie.id}')">Play</button>
                <button class="action-button" onclick="showMoreInfo('${movie.title}')">More Info</button>
            </div>
        `;
...
function playMovie(movieName, movieId) {
    // Fetch video sources from MegaCloud or VidSrc
    fetchVideoSource(movieId)
    .then(videoUrl => {
...
function fetchVideoSource(movieId) {
    // Example URLs for MegaCloud and VidSrc. Replace with actual fetching logic.
    const megaCloudUrl = `https://mega.nz/file/${movieId}#yourMegaCloudKey`; // Replace with actual MegaCloud URL
    const vidSrcUrl = `https://vidsrc.me/video/${movieId}`; // Replace with actual VidSrc URL

    // Check if MegaCloud is accessible first
    return new Promise((resolve, reject) => {
        fetch(megaCloudUrl, { method: 'HEAD' })
        .then(response => {
            if (response.ok) {
                resolve(megaCloudUrl); // If MegaCloud is available
            } else {
                // Fallback to VidSrc if MegaCloud is unavailable
                reject('MegaCloud failed, falling back to VidSrc.');
            }
        })
        .catch(() => {
            reject('MegaCloud failed, falling back to VidSrc.');
    ...
    .catch(() => {
        return Promise.resolve(vidSrcUrl); // Fallback to VidSrc if MegaCloud fails
    });
}
